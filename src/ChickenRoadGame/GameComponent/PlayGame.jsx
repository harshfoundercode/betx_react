...
export default function PlayGame({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (shouldRoast) {
        setIsRoasting(true);  // 🔒 Go button lock karo
  ...
  const redFragmentTimerRef = useRef(null);

  useEffect(() => {
    if (redFragments.length > 0) {
      // Pehle se chal raha timer ho toh cancel mat karo
      if (redFragmentTimerRef.current) return;

      localStorage.setItem("redfragment", "true");
      console.log("🔴 Red fragment activated - localStorage set");

      redFragmentTimerRef.current = setTimeout(() => {
        localStorage.removeItem("redfragment");
        redFragmentTimerRef.current = null;
        console.log("🔴 Red fragment localStorage removed after 2.5 seconds");
      }, 2500);
    }
  }, [redFragments]);
