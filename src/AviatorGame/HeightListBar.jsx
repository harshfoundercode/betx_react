...
function HeightListBar({ hotAirData, betApiHitted, refreshHeightList, setRefreshHeightList }) {
    ...
    return (
        <div className='overflow-x-auto hide-scrollbar w-full flex items-center gap-1.5 sm:gap-2 py-1'>
            {getData?.map((item, i) => (
                <div 
                    key={i} 
                    className={`bg-blackAviator2 rounded-full px-2 sm:px-3 py-0.5 text-[10px] sm:text-xsm whitespace-nowrap ${colors[i % colors.length]}`}
                >
                    {item?.price}
                </div>
            ))}
